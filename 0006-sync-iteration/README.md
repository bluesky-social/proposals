...
*This is draft proposal, not the final specification. Some of the details, terminology, and behaviors might change, though we are pretty confident in the big picture.*

One of the core concepts in atproto is Authenticated Transfer of public user content between services. This document summarizes an iteration on this part of the protocol, also called "Sync", which encompasses the repository, firehose, and CAR export mechanisms. The changes should improve and clarify behaviors, while being largely backwards-compatible and low-disruption to the existing network.
...
- new `com.atproto.sync.listReposByCollection` API endpoint to enumerate all accounts which contain a given record type
- variant of `com.atproto.sync.getRepo` to return a verifiable subset of full repository (eg, a specific collection), including relevant records and MST nodes, as a CAR file
...
- services (including relay) can validate full repo MST integrity without storing the full repo (or any of the repo) locally. This makes operating a relay require dramatically less computer resources (specifically fast disk storage space)
- simplifies relay role in the network, moving "mirror" or "archive" roles to a separate service type
...
- backfill of subsets of the network (eg, only specific record types) becomes much more efficient
...
Casual, non-validating firehose consumers, which do not implement `tooBig` handling, should not break. Note that these services could instead be using an un-authenticated mechanism like jetstream.
...
As an example of this distinction, a PDS might emit a stream of commit events for an account which each contain a single record, as if the repo tree contained a single record at any point in time, without tracking deletions or updates. Each commit event could be signed and validate, but they would not combine in to a repository containing multiple records. Non-validating services would never see "deletions", so they would index more and more content, but the indexed application state would not correspond to the current state of the repository.
...
The `com.atproto.sync.subscribeRepos` event stream `#commit` message (aka, the firehose) contains two fields:
...
- `ops` is an array of repository operations, describing the changes reflected in the `blocks`

The MST in the `blocks` is signed, while the `ops` are not. They can be partially cross-verified, in that all the `ops` can be verified against the `blocks`. However, the `blocks` can not be fully verified against the `ops`. For example, it is possible that many records were deleted in the same commit, and that is represented in the `blocks`, but not all the deletion ops are enumerated in the `ops`.
...
To make it possible to validate individual `#commit` messages in isolation, the previous `data` CID is included in the new `prevData` field on the message. This field is not authenticated (signed), and still needs to be verified against the actual previous seen `data` value.
...
- **Discontinuous Commits:** re-transmitted or missing commits, `prevData` does not match the actual previous commit, duplicate `rev`, etc. Often caused by *operational issues*, though might also be implementation bugs.
...
This service also often unpacks commits in to individual record operations. Commits can contain multiple record operations, but many applications process and index records individually. A example of this pattern is Jetstream, which terminates the sync protocol by consuming commit events, and emits record operation events. This is also the step where many applications filter out irrelevant records.
...
The `tooBig` flag on `#commit` messages played a role similar to `#sync` events in the past: it indicated that the commit message was incomplete, and that data would need to fetched out-of-band (eg, via `getRepo`). The `#sync` can fill some of these use cases (such as bulk-updating an account via CAR import), but is an expensive operation and use should be minimized (and rate-limited).
...
A few fields and endpoints will be deprecated, or fully removed if deprecated earlier. No Lexicon-breaking deprecations will be finalized, meaning no "required" fields will be remove.
...
There are a few related changes we are interested in making, but will not be including in this Sync 1.1 project:
